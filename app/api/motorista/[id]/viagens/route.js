  ...
  try {
    ...
    try {
        ...
        viagens = resViagens.rows.map(v => ({
            ...
            // ✅ CAMPOS PARA O CARDVIAGEM:
            dataViagem: v.data_viagem,        // CardViagem precisa desse nome
            data_viagem: v.data_viagem,        // Backup
            
            horarioViagem: v.horario_saida,    // CardViagem precisa desse nome
            horario_saida: v.horario_saida,    // Backup
            horarioConsulta: v.horario_saida,  // Motorista não tem consulta, usa horário de saída
            horario_consulta: v.horario_saida, // Backup
            
            status: v.status,
            
            hospital: v.ubs_destino_nome || v.hospital_destino,  // CardViagem precisa desse nome
            hospital_destino: v.ubs_destino_nome || v.hospital_destino,
            
            motivo: 'Transporte de Pacientes',  // CardViagem precisa desse campo
            medico: `${v.total_passageiros || 0} passageiro(s)`, // CardViagem precisa desse campo
